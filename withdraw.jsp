        ...
        if (accountNumber != null && amountStr != null && !accountNumber.isEmpty() && !amountStr.isEmpty()) {
            try {
                ...
                if (amount <= 0) {
                    ...
                    // We'll need to add a withdrawFromAccount method to DatabaseUtil
                    // For now, we'll use a generic approach
                    Boolean success = false;
                    
                    try {
                        // Try to withdraw from account
                        success = (Boolean) dbUtilClass.getMethod("withdrawFromAccount", String.class, float.class, String.class)
                            .invoke(null, accountNumber, amount, username);
                    } catch (NoSuchMethodException e) {
                        // If the method doesn't exist, we'll implement a basic withdrawal here
                        success = withdrawFromAccountBasic(accountNumber, amount, username);
                    }
                    
                    if (success) {
                        session.setAttribute("success", "Successfully withdrew Rs." + String.format("%.2f", amount) + " from account " + accountNumber);
                        response.sendRedirect("withdraw.jsp");
                        return;
                    } else {
                        session.setAttribute("error", "Failed to withdraw funds. Please check your balance and try again.");
                    }
                }
            } catch (NumberFormatException e) {
                session.setAttribute("error", "Invalid amount entered.");
            } catch (Exception e) {
                session.setAttribute("error", "An error occurred while processing your withdrawal: " + e.getMessage());
                e.printStackTrace();
            }
        } else {
...
%>
<%!
    // Basic withdrawal method in case the DatabaseUtil method doesn't exist
    public boolean withdrawFromAccountBasic(String accountNumber, float amount, String customerId) {
        boolean success = false;
        try {
            Class<?> dbConnClass = Class.forName("DBConnection");
            Object dbConn = dbConnClass.newInstance();
                ...
                if (currentBalance >= amount) {
                    ...
                    if (rowsAffected > 0) {
                        ...
                        dbUtilClass.getMethod("recordTransaction", String.class, float.class, String.class, String.class, String.class, String.class)
                            .invoke(null, transactionNumber, amount, "Withdrawal", accountNumber, "N/A", customerId);
                        success = true;
                    }
...
<head>
    ...
    <title>Withdraw - Centurion Bank</title>
    ...
    <div class="container">
        <div class="dashboard-grid">
            <div class="main-content">
                <h2 class="section-title"><i class="fas fa-arrow-up"></i> Withdraw Funds</h2>
            ...
            <div class="sidebar">
                <div class="sidebar-section">
                    ...
                    <ul class="nav-links">
                        ...
                        <li><a href="withdraw.jsp" class="active"><i class="fas fa-arrow-up"></i> Withdraw</a></li>
                ...
                <div class="sidebar-section">
                    <h3 class="sidebar-title"><i class="fas fa-info-circle"></i> Withdraw Information</h3>
                    <p>Withdraw funds from your account. Please select an account from the dropdown list and enter the amount you wish to withdraw.</p>
                </div>
            </div>
        </div>
    </div>
